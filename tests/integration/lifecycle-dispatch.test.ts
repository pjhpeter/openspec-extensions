...
`);
    ...
    assert.match(lifecycleText, /worktree 内 change-level review \/ verify 通过且用户验收写入 CHANGE-ACCEPTANCE 后再统一 merge-change/);
...
`);
    ...
    assert.equal(payload.phase, "change_acceptance");
    assert.match(payload.phase_reason, /需先对当前 worktree 范围运行 change-level \/review/);
