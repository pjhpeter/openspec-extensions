...
当前仓库已经建立了 npm/TypeScript 基线；标准运行路径已经切到 `openspec-extensions ...` CLI。Python runtime 目前只作为兼容/回滚窗口保留，不再是标准使用路径。
...
- 运行 TS 测试: `npm test`
- 运行 package smoke: `npm run smoke:package`

迁移发布说明、兼容窗口和回滚策略见 [`docs/ts-migration-release-notes.md`](./docs/ts-migration-release-notes.md)。
...
- archive 阶段建议通过 `openspec-extensions archive change --repo-root . --change "<change>"` 收尾；它会在归档成功后自动清理 change worktree，并在 `mode=branch` 时删除对应 branch
...
- archive 阶段建议直接运行 `openspec-extensions archive change --repo-root . --change "<change>"`；归档成功后会自动清理 change worktree，并在 `mode=branch` 时删除对应 branch
