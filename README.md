...
为所有 spawned subagent 显式指定模型，不要使用默认模型。
```
...
- `worker_worktree.enabled=false` 是默认值，表示 issue 默认运行在共享工作区 `.`，不再默认创建独立 git worktree
- `worker_worktree.enabled=true` 才会默认落到 `.worktree/<change>/<issue>` 这种隔离目录
- 旧仓库如果已经显式写了 `worktree_root` / `worker_worktree.mode` 但还没有 `enabled`，升级后仍按旧的独立 worktree 语义兼容
...
- issue 默认在共享工作区 `.` 中推进，适合串行 issue，避免重复创建 worktree
...
- `subagent_team` 现在已经覆盖：
  ...
  - issue planning 自动接受后立即派发当前 round 的 issue，而不是停在 `control-plane ready`
...
- 当 helper / reconcile 输出 `dispatch_next_issue`、`auto_accept_issue`、`verify_change` 或 `archive_change` 时，coordinator 应立刻继续执行，不应把它们当作 terminal checkpoint
- coordinator 仍然存在，只是不再需要在每个 review gate 之间人工点下一步或人工签字
- 如果 RRA gate 不允许继续，流程会回到 change-level control，而不是盲目前推
- issue 默认在共享工作区 `.` 中推进；只有确实需要并行隔离时，才建议把 `worker_worktree.enabled` 打开
...
- `worker_worktree` 仍保留在契约里，但默认值已经改成共享工作区 `.`；独立 worktree 变成按需开启
