...
7. 只有需要后台脱机继续跑时，才改用外部 worker 会话或 heartbeat
8. worker 只写 issue-local progress 和 run 工件，不直接合并、不直接提交
9. 主会话用 reconcile 收敛状态，输出 round target / normalized backlog / acceptance verdict，再决定 merge、repair 或下一轮 dispatch
10. 所有 issue 都被主会话接受后，再做一轮 change 级 acceptance，然后进入 verify / archive
...
- 为 `<issue-id>` 生成下一轮 worker dispatch / team dispatch 模板
- 为 `<issue-id>` 准备 dispatch，并直接开一个 subagent 执行
- 为 `<issue-id>` 准备 team dispatch，并按开发 / 检查 / 审查小组推进
