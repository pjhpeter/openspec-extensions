...
- eligible issue review is auto-accepted and continues; default change-scoped worktrees merge once after all issues are accepted, worktree-level verify passes, and `runs/CHANGE-ACCEPTANCE.json` records explicit user acceptance
...
- accepted issue code remains in the change worktree through change-level verify; after verify passes, explicit user acceptance writes `runs/CHANGE-ACCEPTANCE.json`, pre-archive closeout merges it into the coordinator repo root, and successful archive should then clean up the reusable worktree
