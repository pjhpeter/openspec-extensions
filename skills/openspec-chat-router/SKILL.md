...
- Code from a change worktree may be merged back to the coordinator repo root only during the pre-archive closeout after the change has passed review, verify, and explicit user acceptance recorded in `runs/CHANGE-ACCEPTANCE.json`; archive must run from the coordinator repo root after that merge. Do not merge worktree code into the coordinator repo root during implementation, issue acceptance, review, verify, or before user acceptance.
