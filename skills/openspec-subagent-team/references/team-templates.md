...
- checker / reviewer 通过后，由 coordinator 统一写 `runs/ISSUE-REVIEW-<issue>.json`，再把 issue progress 收敛到可接受状态；change 级 worktree 等全部 issue 接受并通过 worktree 内 review / verify 后再统一 merge
```
